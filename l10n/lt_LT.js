OC.L10N.register(
    "data_request",
    {
    "sent!" : "išsiųsta!",
    "Already requested, please try again later." : "Jau pateikta užklausa, bandykite dar kartą vėliau.",
    "Request failed" : "Užklausa nepavyko",
    "No administrator could have been contacted." : "Nepavyko susisiekti su jokiu administratoriumi.",
    "Personal data export request" : "Asmeninių duomenų eksportavimo užklausa",
    "Hello %s," : "Sveiki, %s,",
    "The user %s, identified by user id \"%s\", has requested an export of their personal data. Please take action accordingly." : "Vartotojas %s, identifikuojamas pagal vartotojo ID „%s“, pateikė užklausą eksportuoti savo asmens duomenis. Prašome imtis atitinkamų veiksmų.",
    "Account deletion request" : "Paskyros ištrynimo užklausa",
    "The user %s, identified by user id \"%s\", has requested to delete their account. Please take action accordingly." : "Vartotojas %s, identifikuojamas pagal vartotojo ID „%s“, paprašė ištrinti savo paskyrą. Prašome imtis atitinkamų veiksmų.",
    "No administrator has set an email address" : "Joks administratorius nenusistatė el. pašto adreso",
    "Data Request" : "Duomenų užklausa",
    "Request your data from the admins" : "Nusiųskite užklausą administratoriams dėl savo duomenų",
    "Enable your users to request an export or deletion of their data. According options are added to the personal settings section. Administrators will be notified by email about the request." : "Įjunkite savo vartotojams galimybę prašyti duomenų eksportavimo ar ištrinimo. Parinktys parenkamos asmenių nustatymų skiltyje.  Apie prašymų užklausas administratoriai bus informuojami el. paštu.",
    "Account" : "Paskyra",
    "You can request an export of your data or account deletion from the system administrator. This can take up to 30 days." : "Gali užklausti sistemos administratoriaus eksportuoti jūsų duomenis arba ištrinti paskyrą. Tai gali užtrukti iki 30 dienų.",
    "Request data export" : "Užklausti duomenų eksportavimo",
    "Request account deletion" : "Užklausti paskyros ištrynimo"
},
"nplurals=4; plural=(n % 10 == 1 && (n % 100 > 19 || n % 100 < 11) ? 0 : (n % 10 >= 2 && n % 10 <=9) && (n % 100 > 19 || n % 100 < 11) ? 1 : n % 1 != 0 ? 2: 3);");
